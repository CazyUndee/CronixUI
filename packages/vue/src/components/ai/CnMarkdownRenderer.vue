  ...
  methods: {
    ...
    parseMarkdown(text) {
      ...
      html = html.replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) => {
        ...
        return `<pre class="cn-md-code-block"><div class="cn-md-code-lang">${language}</div><code>${this.escapeHtml(code.trim())}</code></pre>`;
      });
      ...
      // Tables
      html = html.replace(/\|(.+)\|\n\|[-| ]+\|\n((?:\|.+\|\n?)+)/g, (_, header, rows) => {
        const headers = header.split('|').filter(Boolean).map(h => `<th class="cn-md-table-th">${h.trim()}</th>`).join('');
        const rowsHtml = rows.trim().split('\n').map(row => {
          const cells = row.split('|').filter(Boolean).map(c => `<td class="cn-md-table-td">${c.trim()}</td>`).join('');
          return `<tr>${cells}</tr>`;
        }).join('');
